...
class ProceduralWorldMap extends PositionComponent
  ...
  late final Paint _dirtRoadPaint = Paint()
    ..color = config.visuals.dirtRoadColor;
  late final Paint _stoneRoadPaint = Paint()
    ..color = config.visuals.stoneRoadColor;
  late final Paint _trainingFieldPaint = Paint()
    ..color = config.visuals.trainingFieldColor;

  // Building Type Paints
  late final Paint _housePaint = Paint()
    ..color = const Color(0xFF8B5A2B); // Wooden brown
  late final Paint _kageOfficePaint = Paint()
    ..color = const Color(0xFFB22222); // Fire red
  late final Paint _academyPaint = Paint()
    ..color = const Color(0xFF4682B4); // Steel blue
  late final Paint _libraryPaint = Paint()
    ..color = const Color(0xFF4B0082); // Mystical indigo
  late final Paint _centralMarketPaint = Paint()
    ..color = const Color(0xFFD27D2D); // Plaza canvas
  late final Paint _hairStorePaint = Paint()
    ..color = const Color(0xFFDA70D6); // Orchid pink
  late final Paint _clothStorePaint = Paint()
    ..color = const Color(0xFF20B2AA); // Light sea green
  late final Paint _supplyStorePaint = Paint()
    ..color = const Color(0xFFCD853F); // Peru tan
  late final Paint _armorStorePaint = Paint()
    ..color = const Color(0xFF708090); // Slate gray
  late final Paint _weaponStorePaint = Paint()
    ..color = const Color(0xFFC0C0C0); // Silver
  late final Paint _libraryStorePaint = Paint()
  ...
  void render(Canvas canvas) {
    ...
    for (final hw in visibleHighways) {
      final paint = hw.material == RoadMaterial.stone
          ? _stoneRoadPaint
          : _dirtRoadPaint;
    ...
    for (final road in visibleRoads) {
      final paint = road.material == RoadMaterial.stone
          ? _stoneRoadPaint
          : _dirtRoadPaint;
